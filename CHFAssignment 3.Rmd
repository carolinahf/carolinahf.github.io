...
head(dat.crime)
```
__In the dataset there are 47 rows and 14 columns. The 14 columnns correspond to the statistics of 14 different variables that were looked at, from unemployment rate to state population size and crime rate. Each row corresponds to 1 of the 47 states for which data was collected.__
...
__Looking at the Residuals vs v plot above, there are no patterns or clumping which would suggest failure of independence, thus the independence assumption is satisfied.__ 
...
__Even though the relationship found above was statistically significant I would not conclude if individuals were to receive more education that crime would be reported more often. ---- .__
